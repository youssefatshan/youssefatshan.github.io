...
function buildFuse() {
    ...
    if (sectionFilter && sectionFilter.value && sectionFilter.value !== 'all') {
        filtered = fullIndex.filter(item => item.section === sectionFilter.value);
    }
    let options = {
        distance: 100,
        threshold: 0.4,
    ...
    fuse = new Fuse(filtered, options);
}
...
sInput.onkeyup = function (e) {
    // rebuild fuse with filtered index on every input
    buildFuse();
    if (fuse) {
        let results;
        if (params.fuseOpts) {
